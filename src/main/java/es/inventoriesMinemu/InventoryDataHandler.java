...
import es.inventoriesMinemu.objects.InventorySyncData;
import es.inventoriesMinemu.objects.InventorySyncTask;
...
public class InventoryDataHandler {
    ...
    public InventoryDataHandler(InventoriesMinemu pd) {
        this.pd = pd;
    }
    ...
    public void setPlayerData(final Player p, DatabaseInventoryData data, InventorySyncData syncData, boolean cancelTask) {
        if (playersInSync.contains(p) == false) {
            ...
            if (InventoriesMinemu.is19Server == false) {
    ...
    public void onDataSaveFunction(Player p, Boolean datacleanup, String syncStatus, ItemStack[] inventoryDisconnect, ItemStack[] armorDisconnect) {
        if (playersDisconnectSave.contains(p) == true) {
            if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                InventoriesMinemu.log.info("Inventory Debug - Save Data - Canceled - " + p.getName());
            }
            return;
        }
        if (datacleanup == true) {
        ...
        if (isPlayerInSync == true) {
            ...
            String armor = "none";
            if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                InventoriesMinemu.log.info("Inventory Debug - Save Data - Start - " + p.getName());
            }
            try {
                if (inventoryDisconnect != null) {
                    if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                        InventoriesMinemu.log.info("Inventory Debug - Set Data - Saving disconnect inventory - " + p.getName());
                    }
                    inv = encodeItems(inventoryDisconnect);
                } else {
                    if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                        InventoriesMinemu.log.info("Inventory Debug - Set Data - Saving inventory - " + p.getName());
                    }
    ...
    private void setInventory(final Player p, DatabaseInventoryData data, InventorySyncData syncData) {
        if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
            InventoriesMinemu.log.info("Inventory Debug - Set Data - Start- " + p.getName());
        }
        if (data.getRawInventory().matches("none") == false) {
            if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                InventoriesMinemu.log.info("Inventory Debug - Set Data - Loading inventory - " + p.getName());
            }
            try {
                p.getInventory().setContents(decodeItems(data.getRawInventory()));
                if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                    InventoriesMinemu.log.info("Inventory Debug - Set Data - Inventory set - " + p.getName());
                }
            } catch (Exception e) {
                e.printStackTrace();
                if (syncData.getBackupInventory() != null) {
                    if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                        InventoriesMinemu.log.info("Inventory Debug - Set Data - Loading backup inventory - " + p.getName());
                    }
                    ...
                    if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                        InventoriesMinemu.log.info("Inventory Debug - Set Data - No backup inventory found! - " + p.getName());
                    }
                }
            }
        } else {
            if (pd.getConfigHandler().getBoolean("Debug.InventorySync") == true) {
                InventoriesMinemu.log.info("Inventory Debug - Set Data - Restoring local inventory - " + p.getName());
            }
            p.getInventory().setContents(syncData.getBackupInventory());
        }
        p.updateInventory();
